...
class SerialShellService(Service):
    ...
    def _command_hid(self, parts):
        mode = self.mode_manager.get_mode("HIDDemo")
        if mode is None:
            self._write_line("HIDDemo mode unavailable.")
            return

        if len(parts) == 1:
            status = mode.get_status()
            self._write_line("HID Toolkit")
            self._write_line("-----------")
            self._write_line("  host: {}".format(status["host"]))
            self._write_line("  state: {}".format(status["state"]))
            self._write_line("  executions: {}".format(status["executions"]))
            self._write_line("  keyboard: {}".format(
                "ready" if status["keyboard"] else "inactive"))
            self._write_line("")
        ...
        if len(parts) == 3 and parts[1].lower() == "host":
            try:
                host = mode.set_host_profile(parts[2])
            except ValueError:
                self._write_line("Unknown HID host: {}".format(parts[2]))
                self._write_line("Valid hosts: linux windows macos")
                return
            self._write_line("HID host: {}".format(host))
            return
